...
void toggleGameState() {
  ...
  //toggle demo
  if ((arduboy.justPressed(UP_BUTTON) || arduboy.justPressed(DOWN_BUTTON)) && arduboy.pressed(A_BUTTON)) {
    if (demo) demo = false;
    else {
      twoplayer = false;
      demo = true;
  ...
  //toggle twoplayer
  if ((arduboy.justPressed(UP_BUTTON) || arduboy.justPressed(DOWN_BUTTON)) && !demo) {
    sound.tone(523, 200);
...
void gameScreen() {
  if(demo) arduboy.setFrameRate(45);
      ...
      score();
      //Pause
      if (arduboy.justPressed(LEFT_BUTTON)) gamestate = 4;
}
void setup() {
  arduboy.begin();
...
void loop() {
  ...
  switch ( gamestate ) {
    case 0:
      ...
      if (arduboy.justPressed(RIGHT_BUTTON)) gamestate = 1;
      printgamemode();
      toggleGameState();
      break;
    case 1:
